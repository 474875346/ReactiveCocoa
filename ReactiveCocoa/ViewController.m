...
@property(nonatomic,retain)RACCommand *conmmand;
@end
...
- (void)viewDidLoad {
    ...
    [RACObserve(_VIEW, center) subscribeNext:^(id x) {
...
//4. RACSubject:RACSubject:信号提供者，自己可以充当信号，又能发送信号。
...
-(void)initRACSubjectAndRACReplaySubject {
    // 1.创建信号
...
//7.RACSequence:RAC中的集合类，用于代替NSArray,NSDictionary,可以使用它来快速遍历数组和字典。
//使用场景：1.字典转模型
//RACSequence和RACTuple简单使用
-(void)initRACSequenceAndRACTuple {
    // 1.遍历数组
...
//8.RACCommand:RAC中用于处理事件的类，可以把事件如何处理,事件中的数据如何传递，包装到这个类中，他可以很方便的监控事件的执行过程。
//使用场景:监听按钮点击，网络请求
//RACCommand简单使用
-(void)initRACCommand {
    // 一、RACCommand使用步骤:
...
//9。RACMulticastConnection:用于当一个信号，被多次订阅时，为了保证创建信号时，避免多次调用创建信号中的block，造成副作用，可以使用这个类处理。
//使用注意:RACMulticastConnection通过RACSignal的-publish或者-muticast:方法创建.
//RACMulticastConnection简单使用:
-(void)initRACMulticastConnection {
